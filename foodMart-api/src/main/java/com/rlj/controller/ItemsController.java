...
public class ItemsController extends BaseController{
    ...
    @GetMapping("/search")
    //前端的那个请求（serverUrl+'/items/seaech?keywords='+keywords+"&sort"+sort+&page="+page+"&pageSize="+pageSize,{}）
    //这里的参数都是请求参数，而不是像之前是路径参数，所以这里@PathVariable--->@RequestParam
    public IMOOCJSONResult search(@RequestParam String keywords,@RequestParam String sort,
                                    @RequestParam Integer page,@RequestParam Integer pageSize) {
        //可留可不留，留下来就要求keywords必须有值才让用，若不留那么keywords即使没值也会查询全部
